...
import {
  ...
  DollarSign, Users, Building, Percent, RefreshCw
} from 'lucide-react';
...
const ContoEconomico = () => {
  ...
  const handleChange = (e) => {
    dispatch({ type: 'UPDATE_CE', payload: { [e.target.name]: parseFloat(e.target.value) || 0 } });
  };
  ...
  const costiServizi = sum(
    ce.serviziEsterniAmm, ce.utenzeTel, ce.consulenze,
    ce.assicurazioni, ce.manutRiparazioni, ce.serviziMarketing
  );
  ...
  const ebit = ebitda - ammortamenti; // A - B
  ...
  return (
    <div className="page-wrapper">
      ...
      <div className="grid-4" style={{ marginBottom: '2rem' }}>
        ...
        <KpiCard label="EBITDA" value={ebitda} color={ebitda >= 0 ? 'green' : 'red'}
          sub={`Margine: ${marginePct}%`} icon={<TrendingUp size={18}/>} />
        <KpiCard label="EBIT" value={ebit} color={ebit >= 0 ? 'cyan' : 'red'} icon={<Minus size={18}/>} />
        <KpiCard label="Utile d'Esercizio" value={utileEsercizio} color={utileEsercizio >= 0 ? 'green' : 'red'}
          sub={`ROS: ${utilePct}%`} icon={<Percent size={18}/>} />
      </div>
